...
- [x] Requirement acceptance flow standardized (`tests/load/acceptance_suites.yaml` defines acceptance suites; `tests/load/run_requirement_acceptance.ps1` is the one-command acceptance trigger; `tests/load/requirement_acceptance.py` writes suite-level acceptance summaries so current requirement sign-off no longer depends on manually selecting benchmark profiles)
- [x] Requirement acceptance finalization hardened (`tests/load/requirement_acceptance.py` now rejects non-boolean `summary.passed` values and only writes `acceptance-summary.*` when the benchmark run itself succeeds; `tests/load/run_requirement_acceptance.ps1` records benchmark and acceptance exit codes in `acceptance-run-meta.json`)
